 ...
 * metabase_odbc.php
 *
 * @(#) $Header: /opt2/ena/metal/metabase/metabase_odbc.php,v 1.34 2010/08/08 06:22:51 mlemos Exp $
 *
 */
...
class metabase_odbc_class extends metabase_database_class
{
	...
	Function DoQuery($query,$first=0,$limit=0,$prepared_query=0)
	{
		...
		{
			$r=intval($result);
			$this->current_row[$r]=-1;
			if(!strcmp(strtolower(strtok(ltrim($query)," \t\n\r")),"select"))
			{
				$this->current_row[$r]=-1;
				if($limit>0)
					$this->limits[$r]=array($first,$limit,0);
				$this->highest_fetched_row[$r]=-1;
			}
			else
			{
	...
	Function FreeResult($result)
	{
		...
		UnSet($this->result_columns[$result_value]);
	...
	{
		if($this->CheckNestedTransaction(1, $auto_commit, $allow))
			return($allow);
	...
	{
		if($this->CheckNestedTransaction(0, 1, $allow))
			return($allow);
	...
	{
		if($this->CheckNestedTransaction(0, 0, $allow))
			return($allow);
